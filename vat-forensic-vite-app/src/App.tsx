      ...
      <main className="layout">
        ...
        <div className="right-column">
          <div className="metrics-grid">
            ...
            <MetricCard title="Control codes" value={controlCodesText.replaceAll(",", " · ")} subtitle="Revenue vs purchase parity check" />
          </div>
          ...
          {analysis ? (
            <>
              ...
              <SectionCard title="Manual review blockers">
                <DataTable columns={["Record ID", "Amount", "Decision", "Exceptions", "Trace"]} rows={analysis.manualReview.map((row) => [row.recordId, formatAmount(row.amount), <DecisionPill key={`${row.recordId}-decision`} label={row.decision.replaceAll("_", " ")} />, row.exceptions.join(" | "), row.trace.join(" | ")])} />
              </SectionCard>
              ...
              <SectionCard title="Full parsed detail register">
                <DataTable columns={["Record ID", "Amount", "Currency", "Decision", "Bucket", "Exceptions", "Trace"]} rows={analysis.detailRows.map((row) => [row.recordId, formatAmount(row.amount), row.currency, <DecisionPill key={`${row.recordId}-full`} label={row.decision.replaceAll("_", " ")} />, row.routeBucket, row.exceptions.join(" | "), row.trace.join(" | ")])} maxRows={100} />
              </SectionCard>
            </>
          ) : (
